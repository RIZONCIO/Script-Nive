...
- **Verificar Erro no Disco e Correção de HD**: Verifica a integridade do disco rígido (HD) e corrige erros encontrados.

- **Subcomando DISM**: Ativa o comando DISM, que baixa e substitui automaticamente arquivos de componentes corrompidos ou ausentes.

- **Reparador de Sistema**: Executa a ferramenta de verificação de arquivos do sistema (SFC), reparando arquivos corrompidos.

- **Limpar HD / Limpar Temp**: Permite a limpeza de arquivos desnecessários ou temporários no HD.

- **Limpar o Cache da DNS**: Executa comandos para limpar o cache DNS, corrigindo possíveis problemas de conexão de rede.

- **Painel de Controle**: Abre o Painel de Controle do Windows para personalização das configurações do sistema.

- **Iniciar o MRT**: Inicia a Ferramenta de Remoção de Software Malicioso (MRT) da Microsoft.

- **Atualizador de Programas**: Faz uma varredura e atualiza os programas instalados no sistema.

- **Reparo Completo do Windows**: Executa uma sequência de comandos para reparo completo do sistema.

### Observações Importantes

1. **Reiniciar após execução:** Após utilizar qualquer comando do script, é recomendado reiniciar o computador.
